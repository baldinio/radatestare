...
//I: n,k - dimensiune combinari, f - fisier text (deschis)
//E: c - contor solutie
void combinari(int n, int k, FILE* f, int &c)
{
	int *x;
	int i; // - pozitia barei
	int vb;
...
void main()
{
	...
	printf("\nCombinari de: ");
	scanf("%d", &dim);
	printf("\nCombinari luate cate: ");
	scanf("%d", &cate);

	sprintf(numef, "Combinari_de_%d_cate_%d.txt", dim, cate);
	fisier = fopen(numef, "wt");
	if (!fisier)
		printf("\nNu se poate crea fisierul text pentru rezultate");
	else
	{
		combinari(dim, cate, fisier, contor);
		printf("\nAm generat %d combinari care sunt in fisierul %s", contor, numef);
		fclose(fisier);
	}
