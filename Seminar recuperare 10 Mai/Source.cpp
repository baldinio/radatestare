...
//I: c - adresa coada, cm - capacitate maxima, co -  capacitate ocupata, x - de adaugat 
//E: cod (0 - succes, 1 - nu se poate), c, co
int push(int* &c, int cm, int &co, int x)
{
	int cod;
...
//I: c - adresa cozii, co - capacitate ocupata
//E: cod (0 - succes, 1 - nu se poate), c, co, x - elementul extras
int popc(int* &c, int &co, int &x)
{
	int cod;
	int i;
...
//extragere informatie din stiva
//I: c - adresa cozii, co - capacitate ocupata
//E: cod (0 - succes, 1 - nu se poate), c, co, x - elementul extras
int pops(int* &c, int &co, int &x)
{
	int cod;
	int i;
...
//parcurgee in latime folosind coada
//I:  a - matrice de adiacenta, n - nr de varfuri, v0 -varf initial
//E: parc - ordine de parcurgere, nr - nr varfuri vizitate, p - vector de parinti
void BF(int n, char** a, int v0, int* &parc, int &nr, int* &p)
{
	int* coada, co;
	int* m;
	int i;
...
void main()
{
	...
	else
	{
		...
		//curatenie
		delete ordine; delete ordine2;
		delete parinti; delete parinti2;
		for (i = 0; i < nm; i++)
			delete tab[i];
		delete tab;
		for (i = 0; i < nv; i++)
